...
- Name: warehouse-flow.spec.ts >> Warehouse Management - Full Flow >> Authenticated flows >> Dashboard shows stats cards
- Location: tests/warehouse-flow.spec.ts:35:9
...
> 29  |       await page.fill('input[type="email"]', ADMIN.email)
  ...
  41  |       await page.locator('div.w-\\[200px\\] >> text=Nguyên liệu').click()
  ...
  109 |     })
  110 | 
  111 |     test('View audit logs', async ({ page }) => {
  112 |       await page.click('text=Audit Logs')
  113 |       await expect(page.locator('th:has-text("Action")')).toBeVisible()
  114 |     })
  115 | 
  116 |     test('Logout', async ({ page }) => {
  117 |       await page.click('text=Đăng xuất')
  118 |       await page.waitForURL('**/login')
  119 |       await expect(page.locator('text=Quản Lý Kho')).toBeVisible()
  120 |     })
  121 |   })
  122 | })
  123 | 
