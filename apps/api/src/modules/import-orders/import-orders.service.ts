...
export class ImportOrdersService {
  ...
  async findAll(q: { page?: string; limit?: string; status?: string }) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.importOrder.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy: { createdAt: 'desc' }, include: { supplier: true, items: { include: { ingredient: true } } } }),
      this.prisma.importOrder.count({ where }),
    ]);
    return { data, meta: { page, limit, total } };
  }

  async create(userId: string, body: { supplier_id: string; note?: string; items: { ingredient_id: string; quantity: number; unit_price: number; expiry_date?: string }[] }) {
  ...
  async approve(id: string, approvedById: string) {
    return this.prisma.$transaction(async (tx) => {
      const order = await tx.importOrder.findUnique({ where: { id }, include: { items: true } });
      if (!order || order.status !== 'PENDING') throw new BadRequestException('Chỉ duyệt phiếu PENDING');

      await tx.importOrder.update({ where: { id }, data: { status: 'COMPLETED', approvedById } });
      for (const item of order.items) {
        await tx.ingredient.update({ where: { id: item.ingredientId }, data: { currentStock: { increment: item.quantity } } });
        await tx.stockTransaction.create({
          data: { ingredientId: item.ingredientId, type: 'IMPORT', quantity: item.quantity, unitPrice: item.unitPrice, totalPrice: item.totalPrice, referenceId: order.id, createdById: approvedById, note: `Nhập kho: ${order.code}` },
        });
      }
  ...
  async reject(id: string, reason?: string) {
    ...
    await this.prisma.importOrder.update({ where: { id }, data: { status: 'REJECTED', note: reason || order.note } });
