...
async function main() {
  ...
  const adminRole = await prisma.role.create({
    data: {
      ...
      permissions: { create: [
        { resource: 'ingredients', action: 'create' }, { resource: 'ingredients', action: 'read' }, { resource: 'ingredients', action: 'update' }, { resource: 'ingredients', action: 'delete' },
        { resource: 'import_orders', action: 'create' }, { resource: 'import_orders', action: 'read' }, { resource: 'import_orders', action: 'approve' },
        { resource: 'stock_exports', action: 'create' }, { resource: 'stock_exports', action: 'read' },
        { resource: 'recipes', action: 'create' }, { resource: 'recipes', action: 'read' }, { resource: 'recipes', action: 'update' },
        { resource: 'users', action: 'create' }, { resource: 'users', action: 'read' }, { resource: 'users', action: 'update' },
        { resource: 'audit_logs', action: 'read' },
        { resource: 'suppliers', action: 'create' }, { resource: 'suppliers', action: 'read' }, { resource: 'suppliers', action: 'update' },
      ] },
    },
  });

  const warehouseRole = await prisma.role.create({
    data: { name: 'Warehouse Staff', code: 'warehouse_staff', permissions: { create: [
      { resource: 'ingredients', action: 'create' }, { resource: 'ingredients', action: 'read' }, { resource: 'ingredients', action: 'update' },
      { resource: 'import_orders', action: 'create' }, { resource: 'import_orders', action: 'read' },
      { resource: 'stock_exports', action: 'create' }, { resource: 'stock_exports', action: 'read' },
  ...
  const kitchenRole = await prisma.role.create({
    data: { name: 'Kitchen Staff', code: 'kitchen_staff', permissions: { create: [
  ...
  await prisma.user.create({ data: { email: 'admin@wms.vn', passwordHash: hash, fullName: 'Nguyễn Văn A', departmentId: departments[0].id, userRoles: { create: { roleId: adminRole.id } } } });
  await prisma.user.create({ data: { email: 'kho@wms.vn', passwordHash: hash, fullName: 'Trần Văn B', departmentId: departments[1].id, userRoles: { create: { roleId: warehouseRole.id } } } });
  await prisma.user.create({ data: { email: 'bep@wms.vn', passwordHash: hash, fullName: 'Lê Thị C', departmentId: departments[2].id, userRoles: { create: { roleId: kitchenRole.id } } } });
  ...
  await prisma.recipe.create({
    data: {
      menuItemId: pho.id, name: 'Phở bò tái', servingSize: 1,
      ingredients: { create: [
