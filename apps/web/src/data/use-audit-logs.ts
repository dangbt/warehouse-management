...
interface ListResponse { data: AuditLog[]; meta: { page: number; limit: number; total: number } }
...
export function useAuditLogs(params?: { user_id?: string; action?: string; resource?: string }) {
  ...
  return useQuery<ListResponse>({ queryKey: QUERY_KEYS.auditLogsList(params as Record<string, string>), queryFn: () => api.get(`/audit-logs?${query}`) })
}
