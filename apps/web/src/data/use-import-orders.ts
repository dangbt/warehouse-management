...
export function useCreateImportOrder() {
  ...
  return useMutation({
    ...
    onSuccess: (newItem) => {
      qc.setQueriesData<ListResponse>({ queryKey: QUERY_KEYS.importOrders }, (old) => old ? { ...old, data: [newItem, ...old.data] } : old)
    },
  })
}
...
export function useApproveImportOrder() {
  ...
  return useMutation({
    ...
    onSuccess: (_, id) => {
      qc.setQueriesData<ListResponse>({ queryKey: QUERY_KEYS.importOrders }, (old) => old ? { ...old, data: old.data.map((o) => o.id === id ? { ...o, status: 'COMPLETED' } : o) } : old)
    },
  })
}
...
export function useRejectImportOrder() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      qc.setQueriesData<ListResponse>({ queryKey: QUERY_KEYS.importOrders }, (old) => old ? { ...old, data: old.data.map((o) => o.id === id ? { ...o, status: 'REJECTED' } : o) } : old)
    },
  })
}
