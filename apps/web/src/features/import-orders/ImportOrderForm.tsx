...
export function ImportOrderForm({ open, onClose, onSave }: Props) {
  const { register, control, handleSubmit, watch, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { supplier_id: '', note: '', items: [{ ingredient_id: '', quantity: 0, unit_price: 0, expiry_date: '' }] },
  })
  ...
  return (
    <WinDialog title="📄 Tạo Phiếu Nhập Kho" open={open} onClose={onClose} width={680}
      footer={
        <>
    ...
    >
      <WinGroupBox title="Thông tin chung">
        <div className="space-y-2">
          <WinSelect label="Nhà cung cấp" {...register('supplier_id')} options={suppliers} error={errors.supplier_id?.message} />
          <WinInput label="Ghi chú" {...register('note')} />
        </div>
      </WinGroupBox>

      <WinGroupBox title="Chi tiết nguyên liệu">
        <button type="button" onClick={() => append({ ingredient_id: '', quantity: 0, unit_price: 0, expiry_date: '' })} className="flex items-center gap-1 text-[11px] text-win-active-title hover:underline cursor-pointer mb-2">
          ...
          <tbody>
            {fields.map((field, i) => (
              <tr key={field.id} className="border-t border-win-grid-border">
                <td className="p-0.5">
                  <select {...register(`items.${i}.ingredient_id`)} className="w-full border border-win-input-border px-1 py-0.5 text-[11px] rounded-sm bg-white">
                    <option value="">--</option>
                    {ingredients.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                </td>
                <td className="p-0.5"><input type="number" step="0.01" {...register(`items.${i}.quantity`)} className="w-full border border-win-input-border px-1 py-0.5 text-[11px] rounded-sm text-right" /></td>
                <td className="p-0.5"><input type="number" {...register(`items.${i}.unit_price`)} className="w-full border border-win-input-border px-1 py-0.5 text-[11px] rounded-sm text-right" /></td>
                <td className="p-0.5 text-right pr-2">{((items[i]?.quantity || 0) * (items[i]?.unit_price || 0)).toLocaleString()}</td>
                <td className="p-0.5"><input type="date" {...register(`items.${i}.expiry_date`)} className="w-full border border-win-input-border px-1 py-0.5 text-[11px] rounded-sm" /></td>
                <td className="p-0.5 text-center">
                  {fields.length > 1 && <button type="button" onClick={() => remove(i)} className="text-win-error hover:opacity-70 cursor-pointer"><Trash2 size={12} /></button>}
                </td>
              </tr>
            ))}
          </tbody>
