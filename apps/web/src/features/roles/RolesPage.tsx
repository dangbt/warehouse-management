...
const RESOURCE_LABELS: Record<string, string> = {
  ...
  import_orders: '📥 Nhập kho',
  stock_exports: '📤 Xuất kho',
  recipes: '🍳 Công thức',
...
export function RolesPage() {
  ...
  const savePermissions = async () => {
    if (!selected) return
    const permissions = Object.entries(perms).filter(([, v]) => v).map(([key]) => {
      const [resource, action] = key.split(':')
      return { resource, action }
    })
    await api.put(`/roles/${selected.id}/permissions`, { permissions })
    setPermOpen(false)
    fetchData()
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <WinToolbar>
        <WinToolbar.Button icon={<Plus size={14} />} label="Thêm Role" onClick={() => setFormOpen(true)} />
      ...
      <WinDataGrid columns={columns} data={data} loading={loading} pagination={{ page: 1, limit: 50, total: data.length }} onRowClick={setSelected} onRowDoubleClick={openPermissions} />

      {/* Create Role Dialog */}
      <RoleForm open={formOpen} onClose={() => setFormOpen(false)} onSave={async (d) => { await api.post('/roles', d); setFormOpen(false); fetchData() }} />

      {/* Permissions Dialog */}
      <WinDialog title={`🔑 Phân quyền: ${selected?.name}`} open={permOpen} onClose={() => setPermOpen(false)} width={600}
        footer={
          <>
...
function RoleForm({ open, onClose, onSave }: { open: boolean; onClose: () => void; onSave: (data: { name: string; code: string; description?: string }) => void }) {
  const [name, setName] = useState('')
  const [code, setCode] = useState('')
  const [desc, setDesc] = useState('')

  useEffect(() => { if (open) { setName(''); setCode(''); setDesc('') } }, [open])

  return (
    <WinDialog title="🆕 Thêm Role" open={open} onClose={onClose} width={400}
      footer={
        <>
          <button onClick={() => { if (name && code) onSave({ name, code, description: desc }) }} className="px-4 py-1 text-xs bg-win-active-title text-white border border-win-active-title rounded-sm min-w-[75px] cursor-pointer">OK</button>
    ...
    >
      <WinGroupBox title="Thông tin Role">
        <div className="space-y-2.5">
          <div className="flex items-center gap-2"><label className="text-[11px] w-24 text-right shrink-0">Tên:</label><input value={name} onChange={(e) => setName(e.target.value)} className="flex-1 border border-win-input-border px-2 py-0.5 text-[11px] rounded-sm outline-none focus:border-win-input-focus" /></div>
          <div className="flex items-center gap-2"><label className="text-[11px] w-24 text-right shrink-0">Mã:</label><input value={code} onChange={(e) => setCode(e.target.value)} className="flex-1 border border-win-input-border px-2 py-0.5 text-[11px] rounded-sm outline-none focus:border-win-input-focus" /></div>
          <div className="flex items-center gap-2"><label className="text-[11px] w-24 text-right shrink-0">Mô tả:</label><input value={desc} onChange={(e) => setDesc(e.target.value)} className="flex-1 border border-win-input-border px-2 py-0.5 text-[11px] rounded-sm outline-none focus:border-win-input-focus" /></div>
        </div>
      </WinGroupBox>
    </WinDialog>
  )
