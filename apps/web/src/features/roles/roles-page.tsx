  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <WinDataGrid
        ...
        data={data ?? []}
        loading={isLoading}
        pagination={{ page: 1, limit: 20, total: data?.length ?? 0 }}
        onRowClick={setSelected}
        onRowDoubleClick={openPermissions}
      />
        ...
        footer={
          <>
            <button
              onClick={() => {
                if (newName && newCode) {
                  createMutation.mutate({ name: newName, code: newCode })
                  setFormOpen(false)
                }
              }}
      ...
      {/* Permissions */}
      <WinDialog
        title={`🔑 Phân quyền: ${selected?.name}`}
        open={permOpen}
                  ...
                  {ACTIONS.map((act) => (
                    <td key={act} className="p-1.5 text-center border-r border-win-grid-border">
                      <input
                        ...
                        onChange={(e) => setPerms({ ...perms, [`${res}:${act}`]: e.target.checked })}
