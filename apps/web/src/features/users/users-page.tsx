...
const columns: Column<UserRow>[] = [
  ...
  { key: 'userRoles', header: 'Role', width: 130, render: (r) => r.userRoles?.map((ur) => ur.role.name).join(', ') },
  {
    ...
    width: 80,
    render: (r) =>
      r.isActive ? <span className="text-win-success">Active</span> : <span className="text-win-error">Inactive</span>,
  },
]
  ...
  return (
    <div className="flex flex-col h-full">
      <WinToolbar>
        ...
        <WinToolbar.Button
          icon={selected?.isActive ? <ShieldOff size={14} /> : <ShieldCheck size={14} />}
          label={selected?.isActive ? 'Khoá' : 'Kích hoạt'}
          danger={selected?.isActive}
          disabled={!selected}
          onClick={() => selected && toggleMutation.mutate(selected.id)}
        />
        <WinToolbar.Separator />
