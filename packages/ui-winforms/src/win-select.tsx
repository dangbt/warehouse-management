...
export const WinSelect = forwardRef<HTMLSelectElement, Props>(({ label, error, options, ...props }, ref) => (
  <div className="flex items-start gap-2">
    ...
    <div className="flex-1">
      <select ref={ref} data-testid={`select-${label}`} {...props} className={`w-full border px-2 py-0.5 text-[11px] rounded-sm outline-none focus:border-win-input-focus bg-white ${error ? 'border-win-error' : 'border-win-input-border'}`}>
